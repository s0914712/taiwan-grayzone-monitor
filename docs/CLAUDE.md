...
| `network-traffic.html` | **網路流量異常監控** — 首屏顯示目前判定與台灣本島／金門／馬祖／澎湖狀態，只保留最近事件；28 天歷史、Chart.js 趨勢、Cloudflare 中斷標註與方法說明預設收合。異常卡優先呈現商船／公務船，背景船流收進歷史細節；`correlation_coverage === 'outside_ais_window'` 時明確說明已超出 AIS 保留範圍。頁面只呈現排程產生的真實觀測，任一 feed 失敗或過期時顯示「資料不足」，不得產生合成事件。JS 在 `js/network-traffic.js`；Chart.js 4.4.0，無 Leaflet。 | `cf_radar.json`, `ioda.json` |
