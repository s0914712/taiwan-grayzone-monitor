...
| `network-traffic.html` | **網路流量異常監控** — Cloudflare Radar 流量／延遲序列（觀測值 vs 季節基線的 Chart.js 折線圖，異常區間以 severity 色帶標示）＋ 異常事件卡（幅度／時數／穩健 z ＋ 異常前 12h 在海纜旁滯留的船隻表）＋ Cloudflare 已知中斷標註 ＋ 4 步驟偵測方法說明。序列以 `.series-tab` 切換。異常卡的線索行有三種狀態：有商船／公務船（橘色 ⚑）、只有漁船（常態背景）、以及 `correlation_coverage === 'outside_ais_window'` 時顯示「此事件早於 AIS 軌跡保留範圍（14 天），無法比對」——**查不到不等於沒有**，不能都講成「無候選」。**示範模式**：`cf_radar.json` 不存在／無 `timestamps`（token 未設定或管線還沒跑過）時，`buildDemo()` 產生合成序列（含一次注入的模擬海纜中斷）並顯示醒目的 DEMO 橫幅——絕不能讓示範資料被誤認為真實觀測。JS 在 `js/network-traffic.js`；Chart.js 4.4.0，無 Leaflet。 | `cf_radar.json`（fallback: 內建示範資料）|
