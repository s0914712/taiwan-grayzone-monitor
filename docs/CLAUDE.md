...
| `cf_radar.json` | Cloudflare Radar 流量／延遲時間序列 + 季節基線 + 異常事件（含關聯船隻）+ 中斷標註 — 由 `generate_dashboard.py` 從 `data/` 複製。`data.json` 內的 `network_anomalies` 已剝掉原始陣列，畫圖要用這個完整檔 | `fetch_cloudflare_radar.py` |
