...
| `../data/vessel_routes/{mmsi}.json` | Per-vessel route files (27,000+). **Not in docs/** — moved out of the Pages artifact (deploys timed out syncing 27k files); the frontend fetches them from `raw.githubusercontent.com` when on GitHub Pages (see `ROUTE_FILE_BASE` in `map-routes.js` / `ship-transfers.html`), relative `../data/` path otherwise | `extract_all_routes.py` |
