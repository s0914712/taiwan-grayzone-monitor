...
|------|---------|-------------|
| `index.html` | Main monitoring dashboard — live map, vessel markers, suspicious list, onboarding tour. **Desktop shell (≥901px) is a vertical flow, not a 100vh grid**: header (brand / `.header-nav` / lang+status) → `.today-brief` 今日態勢 → `.metrics-row` 4 key-metric tiles → `.main-grid` (map pane `minmax(0,1fr)` | 340px sidebar) → ai-summary / data-health / footer. The map lives in `.map-pane` under a `.pane-heading` (title + 圖層 collapse button for the on-map `#layerToggles` panel), its viewport is height-bounded (`clamp(420px, 100vh-360px, 760px)`) so the brief + tiles stay above the fold, and the legend renders as a horizontal `.legend-bar` **below** the map instead of the floating overlay other map pages use. Mobile (≤900px) is untouched: brief / tiles / pane heading are `display:none` and the old map-first shell + drawer + bottom sheet remain. Sidebar order is threat-first: cable status → suspicious (count in the section title as `#suspiciousCount`) → gov vessels → update-info. The AIS 即時統計 / GFW 衛星監測 / AIS 身分變更 / 近期船隻 blocks and the 深度文章 link were **removed to cut information load** — the same numbers live on `statistics.html`, `dark-vessels.html`, `identity-history.html` and the nav "更多" dropdown, and the mobile bottom sheet keeps its own compact 即時統計 tiles. The 12-card bilingual About block moved to `intro.html`; index keeps a one-line `.about-teaser` pointer. | `data.json` |
