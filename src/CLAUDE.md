...
| `generate_og_image.py` | Generate the 1200×630 social-share banner → `docs/og-banner.png`, used site-wide as `og:image` / `twitter:image`. Renders a **real map** of the monitored area: Taiwan/Dongsha territorial baseline (`docs/data/territorial_baseline.json`) as land, real submarine-cable routes (`data/cable-geo.json`), live vessel positions + suspicious vessels (`docs/data.json`), with a left dark-gradient panel for the zh+en title. Committed static asset (not CI-regenerated); rerun to refresh the map or change branding. Needs matplotlib + a CJK font (WenQuanYi/Noto). CLI: `python3 generate_og_image.py [-o docs/og-banner.png]`. |
