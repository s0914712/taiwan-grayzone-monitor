...
Static explainer/pillar pages carry both languages inline (`lang-zh-only` / `lang-en-only` blocks + `data-i18n` spans) and toggle client-side; their English `/en/` URLs are generated by `generate_i18n_pages.py`. Each carries Article(+TechArticle) / BreadcrumbList / FAQPage JSON-LD with `mainEntityOfPage` + `speakable` (`.ai-summary`), a visible `.ai-summary` GEO abstract, a `series-nav` RELATED rail, and a shared bilingual `.topic-cluster` internal-linking footer (inserted before `<footer>`). The glossary (`blog-gray-zone-glossary.html`) is a bilingual `DefinedTermSet`; its terms carry `seeAlso` links to the relevant explainers.
