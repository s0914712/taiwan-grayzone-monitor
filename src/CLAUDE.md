...
| `generate_og_image.py` | Generate the 1200×630 dark-theme social-share banner → `docs/og-banner.png`, used site-wide as `og:image` / `twitter:image`. **Static brand asset** — a one-off committed PNG (not CI-regenerated); rerun only when the tagline/branding changes. Needs matplotlib + a CJK font (WenQuanYi/Noto). CLI: `python3 generate_og_image.py [-o docs/og-banner.png]`. |
